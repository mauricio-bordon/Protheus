...
static function getSaldos(cTipo)
	...
	SELECT B1_COD, RTRIM(B1_DESC) AS DESCRICAO, B1_UM, B2_LOCAL, B2_QATU,
	 BF_LOCALIZ, BF_LOTECTL, coalesce(BF_QUANT,0) as BF_QUANT
	FROM %TABLE:SB1% B1  INNER JOIN  %TABLE:SB2% B2 ON B1.B1_COD=B2.B2_COD
	LEFT JOIN %TABLE:SBF% BF ON B2.B2_COD=BF.BF_PRODUTO   AND BF.D_E_L_E_T_<>'*' AND BF_FILIAL = %XFILIAL:SBF%
	WHERE  B2.D_E_L_E_T_<>'*' AND B2_FILIAL = %XFILIAL:SBF%
		AND B1.D_E_L_E_T_<>'*' AND B1_FILIAL = %XFILIAL:SB1%
	ORDER BY B1_COD, B2_LOCAL,BF_LOCALIZ, BF_LOTECTL
	EndSQL
	u_dbg_qry()
	cKey := ''
	While !(cAlias)->(Eof())
		if cKey <> (cAlias)->B1_COD+(cAlias)->B2_LOCAL
			cKey := (cAlias)->B1_COD+(cAlias)->B2_LOCAL
			...
			aItem[nPos]['local']		:= alltrim((cAlias)->B2_LOCAL)
			aItem[nPos]['quantidade']	:= (cAlias)->B2_QATU
			aItem[nPos]['items']		:= {}
		ENDIF
