...
static function retreceber()
	...
	AND F1_DOC IN (SELECT D1_DOC FROM SD1010 WHERE (D1_COD LIKE 'MP%' OR D1_COD LIKE 'EM%') AND D1_FORNECE=F1_FORNECE AND D_E_L_E_T_<>'*' ) 
	UNION
	SELECT 'R' as 'TIPO',F1_DOC DOC, F1_SERIE SERIE,F1_EMISSAO EMISSAO,A1_NOME,A1_COD,ZS1_ID,ZS1_PLACA 
	FROM %TABLE:SF1% F1
	INNER JOIN %TABLE:SA1% A1
	ON F1_FORNECE=A1_COD
	LEFT OUTER JOIN %TABLE:ZS1% ZS1
	ON F1_DOC=ZS1_DOC
	AND F1_SERIE=ZS1_SERIE
	AND A1_COD=ZS1_FORNEC
	AND ZS1_STATUS<>'F'
	AND ZS1.D_E_L_E_T_<>'*' 
	AND ZS1_FILIAL = %XFILIAL:ZS1%
	WHERE F1.D_E_L_E_T_<>'*' AND F1_FILIAL = %XFILIAL:SF1%
	...
	AND F1_TIPO='B'
   	AND F1_EMISSAO>'20230801'
	AND F1_DOC NOT IN (SELECT ZS1_DOC FROM %TABLE:ZS1%  WHERE F1_DOC=ZS1_DOC	AND F1_SERIE=ZS1_SERIE
	AND A1_COD=ZS1_FORNEC AND ZS1_STATUS='F' AND D_E_L_E_T_<>'*' )
